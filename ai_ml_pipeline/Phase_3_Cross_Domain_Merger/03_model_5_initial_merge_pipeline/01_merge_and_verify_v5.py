# ============================================================
# RoadEye V5 Protocol — Phase 6: The Great Merge & Verification
# ============================================================
# Features:
...
MERGED_DIR = Path(r"D:\RoadEye\dataset_v5_merged")

CLASSES = ["Crack", "Pothole"]

# ── 2. SAFE MERGE LOGIC ───────────────────────────────────────
def copy_and_prefix(source_base, prefix):
...
def perform_merge():
    print("🔄 Phase 1: Merging Datasets with Prefixes...")
    if MERGED_DIR.exists(): shutil.rmtree(MERGED_DIR)
    
    print("   -> Copying Japan V4 (Prefix: 'jp_')...")
    copy_and_prefix(JAPAN_DIR, "jp_")
    print("   -> Copying India V5 (Prefix: 'in_')...")
    copy_and_prefix(INDIA_DIR, "in_")
...
def calculate_merged_stats():
    print("📊 Phase 3: Deep Post-Merge Statistics (Source & Class Breakdown)")
    print("="*75)
    ...
    for i in range(0, sample_count, grid_size):
        ...
        for j in range(grid_size):
            if i+j < sample_count:
                ...
                # Resize with Letterbox instead of raw resize
...
if __name__ == "__main__":
    print("\n🚀 Starting V5 Protocol Final Merge Process\n")
    perform_merge()
    verify_split_isolation()
    calculate_merged_stats()
    generate_yaml()
    visualize_random_samples()
    print("🏆 V5 DATASET IS FULLY PREPARED AND READY FOR FINE-TUNING!")